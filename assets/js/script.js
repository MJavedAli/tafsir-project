function normalizeArabic(str) {
  ...
  const arabicURL = `/data/quran.txt?v=${Date.now()}`;
  const englishURL = "/data/en.hilali.txt";
  const surahNamesURL = "/data/chapter.names.txt";
  const tafsirURL = `/data/tafsir.saadi.txt?v=${Date.now()}`;
  const saadiURL = "/data/saadi.translation.txt";
  const quranSimpleURL = `/data/quran-simple-clean.txt?v=${Date.now()}`;
